...
local maxThreshold = 350   -- tăng giới hạn tối đa lên 350
local areaRadius   = 50
local maxAttempts  = 20

-- Panic mode
local panicking        = false
local panicDuration    = 30    -- seconds
...
local frame = Instance.new("Frame") frame.Name="SettingsFrame" frame.Size=UDim2.new(0,200,0,160) frame.Position=UDim2.new(0,10,0,10) frame.BackgroundTransparency=0.3 frame.BackgroundColor3=Color3.fromRGB(0,0,0) frame.Parent=screenGui
...
local titleLabel = Instance.new("TextLabel") titleLabel.Size=UDim2.new(1,-60,1,0) titleLabel.Position=UDim2.new(0,5,0,0) titleLabel.Text="Settings" titleLabel.TextColor3=Color3.new(1,1,1) titleLabel.BackgroundTransparency=1 titleLabel.Font=Enum.Font.SourceSansBold titleLabel.TextSize=16 titleLabel.Parent=titleBar
local btnMin = Instance.new("TextButton") btnMin.Name="MinimizeButton" btnMin.Size=UDim2.new(0,24,0,24) btnMin.Position=UDim2.new(1,-48,0,0) btnMin.Text="-" btnMin.TextColor3=Color3.new(1,1,1) btnMin.BackgroundTransparency=1 btnMin.Font=Enum.Font.SourceSansBold btnMin.TextSize=18 btnMin.Parent=titleBar
...
-- Thu gọn/Đóng
btnMin.MouseButton1Click:Connect(function() minimized = not minimized; content.Visible = not minimized; frame.Size = minimized and UDim2.new(0,200,0,24) or UDim2.new(0,200,0,160) end)
btnClose.MouseButton1Click:Connect(function() screenGui:Destroy() end)

-- Label & Input
local function createLabel(txt,y)
    local lbl=Instance.new("TextLabel"); lbl.Size=UDim2.new(1,-10,0,24); lbl.Position=UDim2.new(0,5,0,y); lbl.Text=txt; lbl.TextColor3=Color3.new(1,1,1);
    lbl.BackgroundTransparency=1; lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.Font=Enum.Font.SourceSans; lbl.TextSize=14; lbl.Parent=content; return lbl
end
local function createInput(def,y)
    local box=Instance.new("TextBox"); box.Size=UDim2.new(1,-10,0,24); box.Position=UDim2.new(0,5,0,y)
    box.Text=def; box.ClearTextOnFocus=false; box.TextColor3=Color3.new(1,1,1); box.BackgroundColor3=Color3.fromRGB(40,40,40);
    box.BackgroundTransparency=0.2; box.Font=Enum.Font.SourceSans; box.TextSize=14; box.TextXAlignment=Enum.TextXAlignment.Center; box.Parent=content
    local cr=Instance.new("UICorner"); cr.CornerRadius=UDim.new(0,4); cr.Parent=box; return box
end

createLabel("Speed (50-300):",5); local speedBox=createInput(tostring(speed),30)
createLabel("Threshold (10-350):",60); local thresholdBox=createInput(tostring(threshold),85)

speedBox.FocusLost:Connect(function()
    local v=tonumber(speedBox.Text)
    if v then speed=math.clamp(v,minSpeed,maxSpeed) end
    speedBox.Text=tostring(speed)
end)
thresholdBox.FocusLost:Connect(function()
    local v=tonumber(thresholdBox.Text)
    if v then threshold=math.clamp(v,minThreshold,maxThreshold) end
...
local function startPanic()
    ...
    threshold=350  -- đặt khoảng cách an toàn thành 350 khi panic
